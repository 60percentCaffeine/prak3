...
void pack_digit(int* digit, int* pack_taken, int* pack_len, char** pack)
{
...
void pack_num(const int *nums, int nums_len, char **pack, int *pack_len, char **aux, int *aux_len)
{
    ...
    *aux_len = 0;
	...
	for (i = 0; i < nums_len; i++) {
		...
		digits = calc_digits(temp);
        pack_digit(&sign, &pack_taken, pack_len, pack);

        for (j = 0; j < digits; j++)
        {
            pack_digit(&temp, &pack_taken, pack_len, pack);
		}
        ...
        if (aux_free >= 6) {
            ...
            *(*aux + *aux_len) += digits;
            ...
            char right_mask = pow(2, 6 - aux_free) - 1;
...
void unpack_num(char* pack, int pack_len, char* aux, int aux_len)
{
    ...
    while (aux_ptr <= aux_limit)
    {
        ...
        //printf("%d ", digits);
        int sign = read_bin_digits(&pack_ptr, &pack_read, 1, pack_limit);
